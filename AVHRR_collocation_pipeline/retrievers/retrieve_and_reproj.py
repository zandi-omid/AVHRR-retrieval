...
from AVHRR_collocation_pipeline.retrievers.reproject.polar_to_wgs import reproject_vars_polar_to_wgs
...
class AVHRRHybridRetriever:
    ...
    # IO helpers
    # ------------------------------------------------------------------
    def _inputs_from_ds(self, ds: xr.Dataset):
        """
        Common logic: turn an in-memory Dataset into
        (x_stack, tb12, xvec, yvec).
        """
        xvec = ds["x"].values
        yvec = ds["y"].values
        ...
        for v in self.input_vars:
            if v not in ds:
                raise KeyError(f"Input var '{v}' missing from dataset.")
            arrays.append(ds[v].values)

        x = np.stack(arrays, axis=0).astype("float32")
        tb12 = ds["temp_12_0um_nom"].values.astype("float32")

        return x, tb12, xvec, yvec

    def load_group_inputs(self, polar_nc: str | Path, group: str):
        """Existing API: from NetCDF on disk."""
        ds = xr.open_dataset(polar_nc, group=group)
        return self._inputs_from_ds(ds)

    def load_group_inputs_from_dataset(self, ds: xr.Dataset):
        """New API: from in-memory dataset."""
        return self._inputs_from_ds(ds)
    ...
    def run_orbit(
        ...
        *,
        ...
        # --- NH ---
        x_nh, tb12_nh, xvec_nh, yvec_nh = self.load_group_inputs_from_dataset(polar_input_nc, "NH")
        ...
        # --- SH ---
        x_sh, tb12_sh, xvec_sh, yvec_sh = self.load_group_inputs_from_dataset(polar_input_nc, "SH")
    ...
    # ------------------------------------------------------------------
    # Post-processing helpers (mask + clean)
    # ------------------------------------------------------------------
    def mask_preds_with_tb12(
        self,
        preds: dict[str, np.ndarray],
        tb12: np.ndarray,
    ) -> dict[str, np.ndarray]:
        """
        Apply the original swath mask using TB12:
        wherever TB12 is not finite -> set retrievals to NaN.
        """
        mask = ~np.isfinite(tb12)  # True outside swath

        out: dict[str, np.ndarray] = {}
        for k, arr in preds.items():
            a = np.array(arr, copy=True)
            a = np.where(mask, np.nan, a)
            out[k] = a.astype("float32")
        return out
    ...
    def clean_precip(
        ...
        """
        Old clean_dataset logic for the retrieval outputs.
        Clips extremes and removes drizzle.
        """
        out: dict[str, np.ndarray] = {}
        for k in preds.keys():  # typically "mean", "q70", "q75", "q80"
            arr = np.array(preds[k], copy=True)
